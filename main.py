...
# אם הפרונט וה־API על אותו דומיין, CORS לא נדרש, אך לא מזיק
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def load_data():
    ...
    _cats = (
        _df.groupby("Category", dropna=False)
        .size()
...
def categories():
    if df is None:
        raise HTTPException(503, "CSV not loaded")
    return [
        {"category": r.Category, "count": int(r["count"])}
        for _, r in cat_counts.iterrows()
    ]

@app.get("/api/items", response_model=PagedItems)
def list_items(
    q: str | None = None,
...
# ---------- Static frontend ----------
# מגיש את תיקיית frontend כך ש־GET / יחזיר index.html
